...
class ClickGenerator extends CaptchaGenerator implements CaptchaGeneratorInterface
{
    ...
    protected $configs = [
        ...
        'font_family' => __DIR__ . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'style' . DIRECTORY_SEPARATOR . 'zhankukuheiti.ttf',
    ...
    public function config($params = [])
    {
        ...
        if ($this->configs['contents']) {
            $this->configs['font_count'] = mb_strlen($this->configs['contents']);
        }
        return $this;
    }
    ...
    public function check($key, $value, $leeway = 0, $secret = null)
    {
        ...
        foreach ($points as $k => $v) {
            ...
            $isCross = $this->getCross($p1, $p2, $p) * $this->getCross($p3, $p4, $p) >= 0 && $this->getCross($p2, $p3,
                    $p) * $this->getCross($p4, $p1, $p) >= 0;
            if ($isCross) {
                continue;
            } else {
                return false;
